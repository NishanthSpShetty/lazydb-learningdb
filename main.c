...
// allocate page size of 4K, os allocates in block of 4k so, be safe from page
// swap breaking data
...
enum ExecuteResult_t { EXECUTE_TABLE_FULL, EXECUTE_SUCCESS };
...
void print_prompt() { printf("lazydb > "); }
...
PrepareResult prepare_insert(InputBuffer *input_buffer, Statement *statement)
{
    ...
    // null check
    if (keyword == NULL || id_string == NULL || username == NULL ||
        email == NULL) {
    ...
    // avoid buffer overflow when long string enterer
...
ExecuteResult execute_insert(Statement *statement, Table *table)
{
    ...
    // printf("row to insert %d %s %s ", row_to_insert->id,
    // row_to_insert->username, row_to_insert->email); write to table row
    ...
    while (true) {
        ...
        switch (prepare_statement(input_buffer, &statement)) {
            ...
            case PREPARE_STRING_TOO_LONG:
                printf("column too long.\n");
                continue;
            case PREPARE_NEGATIVE_ID:
                printf("id value cannote be negative.\n");
                continue;
        }
